...
describe('Blocks (in general)', () => {
  ...
  it("don't accept 'else'", async () => {
    await expect(engine.parse('{% capture %}{% else %}{% endcapture %}'))
      .to.be.rejectedWith(Liquid.SyntaxError, /tag does not expect else tag/)
  })

  it("don't accept plain 'end'", async () => {
    await expect(engine.parse('{% capture %}{% end %}'))
      .to.be.rejectedWith(Liquid.SyntaxError, /'end' is not a valid delimiter/)
  })

  it('fail if not terminated', async () => {
    await expect(engine.parse('{% capture %}'))
      .to.be.rejectedWith(Liquid.SyntaxError, /tag was never closed/)
  })

  it('fail on odd tags', async () => {
    await expect(engine.parse('{% %}'))
      .to.be.rejectedWith(Liquid.SyntaxError, /was not properly terminated/)
  })

  it('fail on illegal variables', async () => {
    await expect(engine.parse('{{ 2394 '))
      .to.be.rejectedWith(Liquid.SyntaxError, /Variable .* was not properly terminated/)
  })
})

describe('Assign', () => {
  it('assigns a variable', async () =>
    renderTest('.foo.', '{% assign foo = values %}.{{ foo[0] }}.', { values: ['foo', 'bar', 'baz'] })
  )

  it('assigns a variable', async () =>
    renderTest('.bar.', '{% assign foo = values %}.{{ foo[1] }}.', { values: ['foo', 'bar', 'baz'] })
  )

  it('applies filters', async () =>
    renderTest('.BAR.', '{% assign foo = bar | upcase %}.{{ foo }}.', { bar: 'bar' })
  )
})

describe('For', function () {
  it('loops over an array of numbers with spaces', async () =>
    renderTest(' 1  2  3 ', '{%for item in array%} {{item}} {%endfor%}', { array: [1, 2, 3] })
  )

  it('loops over an array of numbers', async () =>
    renderTest('123', '{%for item in array%}{{item}}{%endfor%}', { array: [1, 2, 3] })
  )

  it('loops over an array of letters', async () =>
    renderTest('abcd', '{%for item in array%}{{item}}{%endfor%}', { array: ['a', 'b', 'c', 'd'] })
  )

  it('loops over an array of letters with spaces', async () =>
    renderTest('a b c', '{%for item in array%}{{item}}{%endfor%}', { array: ['a', ' ', 'b', ' ', 'c'] })
  )

  it('loops with empty strings', async () =>
    renderTest('abc', '{%for item in array%}{{item}}{%endfor%}', { array: ['a', '', 'b', '', 'c'] })
  )

  it('loops over ranges', async () =>
    renderTest('1234', '{%for item in (1..4)%}{{item}}{%endfor%}')
  )

  it('loops over hashes/objects', async () =>
  ...
  describe('with reverse', () =>
    it('does not modify the source array', async () => {
      const array = [ 1, 2, 3 ]
      await renderTest('321', '{% for item in array reversed %}{{ item }}{% endfor %}', { array })
...
describe('Comment', () =>
  it("it swallows it's body", () =>
    renderTest('', '{% comment %}This is a comment{% endcomment %}')
  )
)
