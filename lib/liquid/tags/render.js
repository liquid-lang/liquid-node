...
module.exports = class Include extends Liquid.Tag {
  ...
  async renderMultiple (subTemplate, variables) {
    const renders = []
    const value = variables[this.operatorKey]

    for (const item of value) {
      const rendered = await subTemplate.render({ [this.operatorKey]: item })
  ...
  async render (context) {
    ...
    if (this.keyword === 'for') {
      return this.renderMultiple(subTemplate, variables)
    } else {
