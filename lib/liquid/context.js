...
class Context {
  constructor (
    ...
    const ref = engine != null ? new engine.Strainer(this) : void 0
    this.strainer = ref != null ? ref : {}
  ...
  merge (newScope = {}) {
    ...
    for (const key in newScope) {
      if (!hasProp.call(newScope, key)) continue
  ...
  async resolve (key) {
    ...
    if (Liquid.Context.Literals.hasOwnProperty(key)) {
      ...
      return Promise.all([lo, hi])
        .then(args => args.map(Number))
        .then(([loArg, hiArg]) => {
