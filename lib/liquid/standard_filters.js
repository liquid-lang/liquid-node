...
module.exports = {
  ...
  flatten: async function (input) {
    const array = await toIterable(input).toArray()
    return flatten(array)
  },
  join: async function (input, glue = ' ') {
    const array = await this.flatten(input)
    return array.join(glue)
  },
  first: async function (input) {
    return toIterable(input).first()
  },
  last: async function (input) {
