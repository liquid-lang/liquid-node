...
module.exports = (function (superClass) {
  ...
  Block.IsMultiLine = RegExp('^' + Liquid.MultiLineTag.source)

  Block.IsMultiLineIf = RegExp('^' + Liquid.MultiLineIf.source)

  Block.InvalidIf = RegExp(Liquid.InvalidIf.source)
  ...
  Block.prototype.parse = function (tokens, original) {
    ...
    original.map(function(r) {
      if (r.value === v) {
        ...
        token.col = r.col
        token.value = r.value
        token.filename = r.filename
        token.length = r.value.length
      }
    });
  ...
  Block.prototype.parseToken = function (token, tokens, original) {
    var Tag, match, tag
    if (Block.InvalidIf.test(token.value)) {
      throw new Liquid.SyntaxError('invalid `if` syntax')
    } else if (Block.IsMultiLineIf.test(token.value)) {
      // TODO handle multi line if
    } else if (Block.IsMultiLine.test(token.value)) {
