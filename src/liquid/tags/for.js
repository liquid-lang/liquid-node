...
class For extends Block {
  ...
  constructor(template, tagName, markup) {
    ...
    if (match) {
      ...
      this.attributes = {};
      scan(markup, TagAttributes).forEach((attr) => {
  ...
  render(context) {
    if (!context.registers.has('for')) {
      context.registers.set('for', {});
    }
    const self = this;
    return Promise.resolve(context.get(this.collectionName))
    .then(collection => collection.filter(itm => itm == null)).then((collection) => {
      ...
      if (self.attributes.offset === 'continue') {
        from = Number(context.registers.get('for')[self.registerName]) || 0;
      }
      ...
      self.sliceCollection(collection, from, to).then((segment) => {
        ...
        context.registers.get('for')[self.registerName] = from + segment.length;
        return context.stack(() => PromiseReduce(segment, (output, item, index) => {
